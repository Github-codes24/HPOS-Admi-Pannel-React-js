...
const VisitGraph = () => {
    ...
    const { fetchGraphData, candidateVisit, loading, errors} = useCandidates();
    console.log('candidateVisit', candidateVisit);
    ...
    useEffect(() => {
        if (candidateVisit.length > 0) {
            if (timeFrame === 'weekly') {
                setGraphData(getWeeklyData(candidateVisit));
            } else if (timeFrame === 'monthly') {
                setGraphData(getMonthlyData(candidateVisit));
            } else if (timeFrame === 'daily') {
                setGraphData(getDailyData(candidateVisit));
            }
        }
    }, [candidateVisit, timeFrame]);
