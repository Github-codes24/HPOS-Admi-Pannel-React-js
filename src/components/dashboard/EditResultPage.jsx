...
const EditResultPage = () => {
  const { fetchAllCandidates, fetchFilterData, deletePatient, deletePatientData, candidates,
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    const transformedData = transformDataForAPI(values.candidates);
    // Update cervical cancer data
    updateCandidatesResult(transformedData)
      .then(() => {
  ...
  return (
    ...
    <div className="mx-auto mt-5 p-5 bg-white shadow-md">
      ...
      <Formik
        initialValues={{
          candidates: candidates?.length > 0 ? candidates?.map((item) => ({
            _id: item._id || null,
        ...
        {({ values, handleChange, isSubmitting }) => (
          <Form>
            <table className="table-auto w-full mt-5 border-collapse">
              ...
              <tbody>
                {values.candidates?.map((row, index) => (
                  <tr key={index} className="border-b">
                    <td>
                      <Field
                        type="text"
                        name={`candidates[${index}].personalName`}
                    ...
                    <td>
                      <Field
                        as="select"
                        name={`candidates[${index}].maritalStatus`}
                    ...
                    <td>
                      <Field
                        type="text"
                        name={`candidates[${index}].centerCode`}
                    ...
                    <td>
                      <Field
                        as="select"
                        option={getCities}
                        name={`candidates[${index}].location`}
                        // value={row.location}
                        value={row.location}
                        // onChange={handleChange}
                        onChange={(option) => {
                    ...
                    <td>
                      <Field
                        as="select"
                        name={`candidates[${index}].bloodGroup`}
                    ...
                    <td>
                      <Field
                        as="select"
                        name={`candidates[${index}].resultStatus`}
                    ...
                    <td>
                      <Field
                        as="select"
                        name={`candidates[${index}].cardStatus`}
