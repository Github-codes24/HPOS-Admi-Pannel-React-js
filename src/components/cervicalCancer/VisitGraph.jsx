...
const VisitGraph = () => {
    ...
    const { fetchGraphData, cervicalCancerVisit, loading, errors} = useCervicalCancer();
    console.log('cervicalCancerVisit', cervicalCancerVisit);
    ...
    useEffect(() => {
        if (cervicalCancerVisit.length > 0) {
            if (timeFrame === 'weekly') {
                setGraphData(getWeeklyData(cervicalCancerVisit));
            } else if (timeFrame === 'monthly') {
                setGraphData(getMonthlyData(cervicalCancerVisit));
            } else if (timeFrame === 'daily') {
                setGraphData(getDailyData(cervicalCancerVisit));
            }
        }
    }, [cervicalCancerVisit, timeFrame]);
