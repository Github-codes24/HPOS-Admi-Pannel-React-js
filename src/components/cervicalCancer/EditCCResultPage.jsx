...
const EditCCResultPage = () => {
  const { fetchAllCervicalCancer, fetchFilterData, deletePatient, deletePatientData, cervicalCancerData,
    updateCervicalCancerResult
  } = useCervicalCancer();
  const [loading, setLoading] = useState(true);
  const { fetchCities, getCities } = useCities();
  console.log('cervicalCancerData', cervicalCancerData)
  ...
  useEffect(() => {
    fetchAllCervicalCancer().finally(() => setLoading(false));
    fetchFilterData({})
    fetchCities();
  }, []);
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    const transformedData = transformDataForAPI(values.cervicalCancerData);
    // Update cervical cancer data
    updateCervicalCancerResult(transformedData)
      .then(() => {
      ...
      <div className="mx-auto mt-5 p-5 bg-white shadow-md">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold">Cervical Cancer</h1>
          </div>
          {/* <div>
        ...
        <Formik
          initialValues={{
            cervicalCancerData: cervicalCancerData?.length > 0 ? cervicalCancerData?.map((item) => ({
              _id: item._id || null,
          ...
          {({ values, handleChange, isSubmitting }) => (
            <Form>
              <table className="table-auto w-full mt-5 border-collapse">
                ...
                <tbody>
                  {values.cervicalCancerData?.map((row, index) => (
                    <tr key={index} className="border-b">
                      <td>
                        <Field
                          type="text"
                          name={`cervicalCancerData[${index}].personalName`}
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].maritalStatus`}
                      ...
                      <td>
                        <Field
                          type="text"
                          name={`cervicalCancerData[${index}].centerCode`}
                      ...
                      <td>
                        <Field
                          type="text"
                          name={`cervicalCancerData[${index}].age`}
                          value={row.age}
                          onChange={handleChange}
                          className="input input-sm w-full"
                        />
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].gender`}
                          value={row.gender}
                          onChange={handleChange}
                          className="select select-sm w-full"
                        >
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].category`}
                          value={row.category}
                          onChange={handleChange}
                          className="select select-sm w-full"
                        >
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].caste`}
                          value={row.caste}
                          onChange={handleChange}
                          className="select select-sm w-full"
                        >
                      ...
                      <td>
                        <Field
                          as="select"
                          option={getCities}
                          name={`cervicalCancerData[${index}].location`}
                          // value={row.location}
                          value={row.location}
                          // onChange={handleChange}
                          onChange={(option) => {
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].bloodGroup`}
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].resultStatus`}
                      ...
                      <td>
                        <Field
                          as="select"
                          name={`cervicalCancerData[${index}].cardStatus`}
...
export default EditCCResultPage