...
const CervicalCancerPage = () => {
    ...
    const ExportInfoExcel = () => {
        if (cervicalCancerData && cervicalCancerData.length > 0) {
            // Create data array by mapping through the cervical cancer data
            const data = cervicalCancerData?.map(candidate => ({
            ...
            const worksheet = XLSX.utils.json_to_sheet(data);
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, "Cervical Cancer Data");
    
            // Write the Excel file and trigger the download
            const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
            const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
            FileSaver.saveAs(blob, `cervical_cancer_data.xlsx`);
        } else {
            console.error("No cervical cancer data available to export.");
        }
    };
