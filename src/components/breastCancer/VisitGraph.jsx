...
const VisitGraph = () => {
    ...
    const { fetchGraphData, breastCancerVisit, loading, errors} = useBreastCancer();
    console.log('breastCancerVisit', breastCancerVisit);
    ...
    useEffect(() => {
        if (breastCancerVisit.length > 0) {
            if (timeFrame === 'weekly') {
                setGraphData(getWeeklyData(breastCancerVisit));
            } else if (timeFrame === 'monthly') {
                setGraphData(getMonthlyData(breastCancerVisit));
            } else if (timeFrame === 'daily') {
                setGraphData(getDailyData(breastCancerVisit));
            }
        }
    }, [breastCancerVisit, timeFrame]);
