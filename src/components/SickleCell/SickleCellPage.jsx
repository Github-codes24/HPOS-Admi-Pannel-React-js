...
const ExportInfoExcel = () => {
    if (sickleCellData && sickleCellData.length > 0) {
        ...
        const data = sickleCellData?.map(candidate => ({
        ...
        const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
        const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
        FileSaver.saveAs(blob, `sickleCellData_details.xlsx`);
    } else {
    ...
    const onEdit = (item) => {
        ...
        navigate(`/edit-sickle-cell/${item._id}`);
    }

    const onDelete = (item) => deletePatient(item?._id)

    const onEditResult = () => {
        navigate(`/edit-result-SickleCell`);
    }
