...
const ViewSickleCellPage = () => {
    const { fetchSubmiitedSickleCell, submittedSickleCell, fetchSubmittedFilterData } = useSickleCell();
    ...
    const downloadSubmittedCandidate = () => {
      if (submittedSickleCell && submittedSickleCell.length > 0) {
        const data = submittedSickleCell.map(candidate => ({
    ...
    <div className="container mx-auto p-4">
      ...
      <table className="table w-full ">
        ...
        <tbody>
          ...
          {submittedSickleCell?.map((item, index) => (
            <tr key={index}>
...
export default ViewSickleCellPage